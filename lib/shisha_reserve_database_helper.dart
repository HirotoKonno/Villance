...
class ShishaReserveDatabaseHelper {
  static const _databaseName = "ShishaReserveDatabase.db";
  ...
  ShishaReserveDatabaseHelper._privateConstructor();

  static final ShishaReserveDatabaseHelper instance =
      ShishaReserveDatabaseHelper._privateConstructor();
  ...
  Future<bool> checkExistedTime(Map<String, dynamic> row) async {
    Database? db = await instance.database;
    final hourMinutes = row[ShishaReserveDatabaseHelper.hourMinutesString];
    final List<Map<String, dynamic>> maps = await db!.query(tableSauna);
    final data = List.generate(maps.length, (i) {
      return ReservedShisha(
          maps[i][ShishaReserveDatabaseHelper.columnId],
          maps[i][ShishaReserveDatabaseHelper.isToday],
          maps[i][ShishaReserveDatabaseHelper.hourMinutesString]);
    });
  ...
  Future<void> addReserveTime(Map<String, dynamic> row) async {
    Database? db = await instance.database;
    final isToday = row[ShishaReserveDatabaseHelper.isToday];
    final hourMinutes = row[ShishaReserveDatabaseHelper.hourMinutesString];
  ...
  Future<List<ReservedShisha>> getReservedTimes() async {
  ...
  Future<int> deleteTime(String time) async {
    Database? db = await instance.database;
    return await db!
        .delete(tableSauna, where: '$hourMinutesString = ?', whereArgs: [time]);
  }
